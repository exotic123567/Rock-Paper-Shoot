...
/// <summary>
/// Object Pool for bullet objects, used for controlling how objects are spawned by Netcode.
/// Similar to the NetworkObjectPool, but specifically for bullet objects.
/// </summary>
public class BulletNetworkObjectPool : NetworkBehaviour
{
    private static BulletNetworkObjectPool _instance;

    public static BulletNetworkObjectPool Singleton { get { return _instance; } }

    [SerializeField]
    List<BulletPoolConfigObject> PooledPrefabsList;
    ...
    public void OnValidate()
    {
        for (var i = 0; i < PooledPrefabsList.Count; i++)
        {
            ...
            if (prefab != null)
            {
                Assert.IsNotNull(prefab.GetComponent<NetworkObject>(), $"{nameof(BulletNetworkObjectPool)}: Pooled prefab \"{prefab.name}\" at index {i.ToString()} has no {nameof(NetworkObject)} component.");
            }
        }
    }
    ...
    private void RegisterPrefabInternal(GameObject prefab, int prewarmCount)
    {
        ...
        NetworkManager.Singleton.PrefabHandler.AddHandler(prefab, new BulletPooledPrefabInstanceHandler(prefab, this));
    }
...
[Serializable]
struct BulletPoolConfigObject
{
    public GameObject Prefab;
    public int PrewarmCount;
}

class BulletPooledPrefabInstanceHandler : INetworkPrefabInstanceHandler
{
    GameObject m_Prefab;
    BulletNetworkObjectPool m_Pool;

    public BulletPooledPrefabInstanceHandler(GameObject prefab, BulletNetworkObjectPool pool)
    {
        m_Prefab = prefab;
        m_Pool = pool;
    }
